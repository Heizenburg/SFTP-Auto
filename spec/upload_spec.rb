  ...
  describe '#validate_local_directory' do
    ...
    context 'when the local directory is not specified' do
      it 'raises an error' do
        ...
        expect do
          sftp_uploader.send(:validate_local_directory)
        end.to raise_error(RuntimeError, 'Error: local directory is not specified.')
      end
    end
  end

  describe '#clients_to_cycle' do
    let(:client_list) { %w[client1 client2 client3 client4 client5] }
    ...
    context 'when only one argument is provided' do
      it 'returns the specified number of clients' do
        expect(clients_to_cycle(client_list, 3)).to eq(%w[client1 client2 client3])
      end
    end

    context 'when two arguments are provided' do
      it 'returns a range of clients' do
        expect(clients_to_cycle(client_list, 2, 4)).to eq(%w[client2 client3 client4])
      end
    end
  end
end
