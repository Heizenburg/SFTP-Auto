...
class TestClientsToCycle < Minitest::Test
  ...
  describe 'clients_to_cycle' do
    let(:client_list) { [['client1', 'remote_location1'], ['client2', 'remote_location2'], ['client3', 'remote_location3']] }
  
    it 'should return the first n clients if there are two arguments' do
      ARGV.replace ['upload', '2']
      expect(clients_to_cycle(client_list)).to eq([['client1', 'remote_location1'], ['client2', 'remote_location2']])
    end
  
    it 'should return the clients within the specified range if there are three arguments' do
      ARGV.replace ['upload', '2', '3']
      expect(clients_to_cycle(client_list)).to eq([['client2', 'remote_location2'], ['client3', 'remote_location3']])
    end
  ...
  describe 'print_remote_entries' do
    ...
    let(:client) { 'client1' }
    let(:entry1) { double('entry1', name: 'file1', longname: 'file1', attributes: double('attributes', directory?: false, size: 1024)) }
    let(:entry2) { double('entry2', name: 'file2', longname: 'file2', attributes: double('attributes', directory?: true, size: 0)) }
    let(:entry3) { double('entry3', name: 'file3', longname: 'file3', attributes: double('attributes', directory?: false, size: 2048)) }
    ...
    it 'should print the correct output for a recent non-client file' do
      ...
      expect { print_remote_entries(session, remote_location, client) }.to output(/file1.*NEW FILE DOES NOT BELONG HERE/).to_stdout
    end
    ...
    it 'should print the correct output for a non-recent non-client file' do
      ...
      allow(self).to receive(:client_file?).with('file1', client).and_return(false)
      expect { print_remote_entries(session, remote_location, client) }.to output(/file1.*FILE DOES NOT BELONG HERE/).to_stdout
    end
  ...
  describe 'get_range' do
    ...
    let(:clients) { [['client1', 'remote_location1'], ['client2', 'remote_location2'], ['client3', 'remote_location3']] }
    ...
    it 'should return the range if the user provides a range' do
      ...
      expect(get_range(prompt, clients)).to eq(['2', '3'])
    end
  end
  ...
  describe 'print_client_information' do
    ...
    it 'should print the correct output when there are two arguments' do
      ARGV.replace ['upload', '2']
      expect { print_client_information(index, client, remote_location) }.to output(/\[1: client1\] remote_location1/).to_stdout
    end
  
    it 'should print the correct output when there are three arguments' do
      ARGV.replace ['upload', '2', '3']
      expect { print_client_information(index, client, remote_location) }.to output(/\[3: client1\] remote_location1/).to_stdout
    end
  end
